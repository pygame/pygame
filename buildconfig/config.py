...
def is_msys2():
    ...
    return "MSYSTEM" in os.environ and re.match(
        r"MSYS|MINGW.*|CLANG.*|UCRT.*", os.environ["MSYSTEM"]
    )
...
def prepdep(dep, basepath):
    ...
    if dep.name.startswith("COPYLIB_"):
        ...
        dep.line = (
            dep.name + " =" + "".join(incs) + "".join(lids) + " " + dep.cflags + libs
        )


def writesetupfile(deps, basepath, additional_lines):
    """create a modified copy of Setup.SDLx.in"""
    sdl_setup_filename = os.path.join(BASE_PATH, "buildconfig", "Setup.SDL2.in")

    with open(sdl_setup_filename, "r") as origsetup, open(
        os.path.join(BASE_PATH, "Setup"), "w"
    ) as newsetup:
        line = ""
...
def main(auto=False):
    ...
    if "-sdl1" in sys.argv:
        raise SystemExit(
            """Building PyGame with SDL1.2 is no longer supported.
Only SDL2 is supported now."""
    ...
    if os.path.isfile("Setup"):
        if auto:
            ...
            shutil.copyfile(
                os.path.join(BASE_PATH, "Setup"), os.path.join(BASE_PATH, "Setup.bak")
            )

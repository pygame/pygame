...
class PixelArrayTypeTest(unittest.TestCase, TestMixin):
    def test_compare(self):
        ...
        ar3 = ar.compare(ar2)
        self.assertIsInstance(ar3, pygame.PixelArray)
    ...
    def test_contains(self):
        for bpp in (8, 16, 24, 32):
            ...
            ar = pygame.PixelArray(sf)
            self.assertIn((0, 0, 0), ar)
            self.assertIn((255, 255, 255), ar)
            self.assertNotIn((255, 255, 0), ar)
            self.assertNotIn(0x0000FF, ar)

            # Test sliced array
            self.assertIn((0, 0, 0), ar[8])
            self.assertIn((255, 255, 255), ar[8])
            self.assertNotIn((255, 255, 0), ar[8])
            self.assertNotIn(0x0000FF, ar[8])

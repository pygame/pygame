...
if (sys.version_info[0] == 2 and sys.version_info[1] < 5):
    from pygame2.threads.Py25Queue import Queue, Empty
...
def benchmark_workers(a_bench_func = None, the_data = None):
    ...
    #  first find some variables so that using 0 workers takes about 1.0
    #  seconds.
    #  then go from there.
    #
    ...
    import pygame2
    import pygame2.sdl.transform
    import time

    if not a_bench_func:
        def doit(x):
            return pygame2.sdl.transform.scale(x, (544, 576))
    else:
        doit = a_bench_func
    ...
    if not the_data:
        thedata = []
        for x in range(10):
            thedata.append(pygame2.sdl.Surface((155,155), 0, 32))
    else:
        thedata = the_data
...
class FuncResult:
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            # TODO: reactivate e for 2.x and 3.0
            self.exception = None 
            if self.errback:
                self.errback(self.exception)


def tmap(f, seq_args, num_workers = 20, worker_queue = None, wait = True,
         stop_on_error = True):
    ...
    if wait:
        ...
        # see if there were any errors.  If so raise the first one.
        # This matches map behaviour.

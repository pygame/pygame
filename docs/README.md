...
- clone the pygame repository, navigate to it in the terminal
...
You can launch this by clicking on index.html or by running __main__.py in
`docs/`.

There is also a `docs --fullgeneration` or `docs --f` command for regenerating
everything regardless of whether Sphinx it should be regenerated. This is
useful when editing the theme CSS.
...
to convert these rst files into html, which are then hosted on the pygame
website.

...
.. c:function:: int pg_IntFromSeqIndexEx(PyObject *obj, int index, int *val, const char *msg)
   ...
   Faster alternative to ``pg_IntFromObjIndex`` as it assumes *obj* to be a Sequence, uses
   ``pg_IntFromObjEx`` for faster item->int conversion and does not clear errors.

   Use this function ONLY when you know *obj* implements the Sequence protocol or
   check it with PySequence_Check() before calling ``pg_IntFromSeqIndexEx``.

...
#warning Add mode stuff.
    ...
    if (!state->icon)
    {
        ...
        {
            SDL_SetColorKey (pgSurface_AsSurface (state->icon),
                             SDL_TRUE,
                             0);
        }
    }
...
pg_set_mode (PyObject* self, PyObject* arg)
{
    ...
    if (!icon_was_set)
    {
        ...
        {
            SDL_SetColorKey (pgSurface_AsSurface (iconsurf), SDL_SRCCOLORKEY,
                             0);
...
pg_set_palette (PyObject* self, PyObject* args)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if(!pg_IntFromObjIndex(item, 0, &r) || !pg_IntFromObjIndex(item, 1, &g)

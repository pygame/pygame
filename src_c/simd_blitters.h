...
#if defined(HAVE_IMMINTRIN_H) && !defined(SDL_DISABLE_IMMINTRIN_H)
static void
blit_blend_rgba_mul_simd_avx2(SDL_BlitInfo * info)
{
    int             n;
    int             width = info->width;
    int             height = info->height;
    ...
    __m128i mm_src, mm_dst, mm_zero, mm_two_five_fives;
    __m256i mm256_src, mm256_srcA, mm256_srcB, mm256_dst, mm256_dstA, mm256_dstB, mm256_shuff_mask_A, mm256_shuff_mask_B, mm256_two_five_fives;

    mm256_shuff_mask_A = _mm256_set_epi8(0x80,23,0x80,22,0x80,21,0x80,20,0x80,19,0x80,18,0x80,17,0x80,16,0x80,7,0x80,6,0x80,5,0x80,4,0x80,3,0x80,2,0x80,1,0x80,0);
    mm256_shuff_mask_B = _mm256_set_epi8(0x80,31,0x80,30,0x80,29,0x80,28,0x80,27,0x80,26,0x80,25,0x80,24,0x80,15,0x80,14,0x80,13,0x80,12,0x80,11,0x80,10,0x80,9,0x80,8);

    mm_zero = _mm_setzero_si128();
    mm_two_five_fives = _mm_set_epi64x(0x00FF00FF00FF00FF, 0x00FF00FF00FF00FF);

    mm256_two_five_fives = _mm256_set_epi8(0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,
                                           0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,
                                           0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,
                                           0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF);

    if (((width % 8) == 0) && ((srcskip % 8) == 0) && ((dstskip % 8) == 0)) {
        width = width / 8;
        srcskip = srcskip / 8;
        dstskip = dstskip / 8;
        ...
        while (height--)
        {
            LOOP_UNROLLED4(
            {
                mm256_src = _mm256_load_si256(srcp256);
                mm256_dst = _mm256_load_si256(dstp256);
    ...
    else if (((width % 2) == 0) && ((srcskip % 2) == 0) && ((dstskip % 2) == 0)) {
        width = width / 2;
        srcskip = srcskip / 2;
        dstskip = dstskip / 2;
        while (height--)
